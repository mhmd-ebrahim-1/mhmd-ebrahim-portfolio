    ...
    >
      ...
      <div className="flex flex-col items-center">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 z-10"
          ...
          {isEdu
            ? <GraduationCap size={16} style={{ color: item.current ? '#00f5d4' : 'rgba(255,255,255,0.3)' }} />
            : <Briefcase size={16} style={{ color: item.current ? '#00f5d4' : 'rgba(255,255,255,0.3)' }} />
          }
        </div>
      ...
      <div className="flex-1 pt-1 pb-2">
        <div className="flex items-start justify-between mb-1">
          <h3 className="font-display font-semibold text-white text-lg">{item.title || item.degree}</h3>
          {item.current && (
            <span className="px-2 py-0.5 rounded-full font-mono text-xs flex items-center gap-1.5"
        ...
        <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.5)', fontFamily: 'DM Sans' }}>
          {item.description}
        </p>
        {item.skills && (
  ...
  return (
    <div className="page-transition min-h-screen pt-24 pb-20 px-10">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <div>
            <p className="font-mono text-xs mb-4" style={{ color: '#00f5d4', letterSpacing: '0.12em' }}>// 05 — RÉSUMÉ</p>
            <h1 className="font-display text-6xl font-bold mb-4" style={{ letterSpacing: '-2px' }}>
              My <span className="grad-text">CV</span>
            </h1>
            <p className="text-base" style={{ color: 'rgba(255,255,255,0.4)', fontFamily: 'DM Sans' }}>
              Experience, education, and skills — all in one place.
            </p>
          </div>
          <motion.button
        ...
        {/* CV Card (printable style) */}
        <div className="glass rounded-3xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>

          {/* CV Header */}
          <div className="p-10 pb-8" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,245,212,0.02)' }}>
              ...
              <div className="w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0"
                ...
                <img src="/profile.jpg" alt={PROFILE.name} className="w-full h-full object-cover object-top"
                  onError={e => {
                    ...
                    e.target.parentElement.innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:rgba(0,245,212,0.1);color:#00f5d4;font-family:Syne,sans-serif;font-size:24px;font-weight:700">ME</div>'
                  }}
                />
              </div>
              <div className="flex-1">
                <h2 className="font-display text-3xl font-bold text-white mb-1">{PROFILE.fullName}</h2>
                <p className="text-base mb-3" style={{ color: '#00f5d4', fontFamily: 'DM Sans' }}>{PROFILE.title}</p>
                <div className="flex flex-wrap gap-4">
                  {[
                    ...
                    <a key={text} href={href} target={href ? '_blank' : undefined} rel="noopener noreferrer"
                      className="flex items-center gap-1.5 font-mono text-xs"
                      style={{ color: 'rgba(255,255,255,0.45)', textDecoration: 'none' }}>
                      <Icon size={12} style={{ color: '#00f5d4' }} />
                      {text}
                    </a>
                  ))}
                </div>
            ...
            {/* Summary */}
            <p className="mt-6 text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.55)', fontFamily: 'DM Sans', maxWidth: '700px' }}>
              {PROFILE.bio}
            </p>
          </div>
          ...
          <div className="grid grid-cols-5 divide-x" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
            ...
            <div className="col-span-3 p-10">
              ...
              <div className="mb-10">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h3 className="font-display font-semibold text-white text-lg">Experience</h3>
                </div>
              ...
              <div>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="font-display font-semibold text-white text-lg">Learning Activities</h3>
                </div>
                {LEARNING_ACTIVITIES.map((a, i) => (
                  <motion.div key={a.id} className="mb-5 pl-4"
                    style={{ borderLeft: '2px solid rgba(251,146,60,0.2)' }}
                    initial={{ opacity: 0, x: -10 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.1 }}
                  >
                    <p className="font-display font-semibold text-white text-sm">{a.title}</p>
                    <p className="text-xs mb-1" style={{ color: '#fb923c' }}>{a.org}</p>
                    <p className="font-mono text-xs mb-2" style={{ color: 'rgba(255,255,255,0.3)' }}>{a.period}</p>
                    <p className="text-xs" style={{ color: 'rgba(255,255,255,0.4)', fontFamily: 'DM Sans' }}>{a.description}</p>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div className="col-span-2 p-10">
              ...
              ].map(({ section, color, skills }) => (
                <div key={section} className="mb-7">
                  <p className="font-mono text-xs mb-3" style={{ color, letterSpacing: '0.08em' }}>{section.toUpperCase()}</p>
                  <div className="flex flex-wrap gap-2">
                    {skills.map(s => (
                      <span key={s} className="font-mono text-xs px-2 py-1 rounded-md"
                        style={{ background: `${color}0a`, border: `1px solid ${color}20`, color: 'rgba(255,255,255,0.6)' }}>
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mt-8">
                ...
                {[
                  ...
                  <div key={lang} className="flex justify-between items-center mb-3">
                    <span className="text-sm" style={{ color: 'rgba(255,255,255,0.7)', fontFamily: 'DM Sans' }}>{lang}</span>
                    <span className="font-mono text-xs px-2 py-0.5 rounded"
                      style={{ background: 'rgba(0,245,212,0.08)', color: '#00f5d4' }}>{level}</span>
                  </div>
                ))}
              </div>
              ...
              {/* Key tools */}
              <div className="mt-8">
                <h3 className="font-display font-semibold text-white text-lg mb-4">Key Tools</h3>
                <div className="flex flex-wrap gap-2">
                  {TOOLS.slice(0, 8).map(t => (
                    <span key={t.name} className="font-mono text-xs px-2 py-1 rounded-md"
