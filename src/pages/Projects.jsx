...
function ProjectCard({ proj, index }) {
  ...
  const image = `/My_Portfolio/projects/${proj.slug}.svg`
  ...
  return (
    <motion.article
      ...
      style={{ border: `1px solid ${hovered ? proj.accent + '30' : 'rgba(255,255,255,0.06)'}`, transition: 'border-color .3s' }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <div className="h-52 relative overflow-hidden bg-[#0b0b14]">
        <img src={image} alt={`${proj.title} project preview`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#080810]/75 via-transparent to-transparent pointer-events-none" />
        {proj.featured && (
          <span className="absolute top-4 left-4 px-3 py-1 rounded-full font-mono text-xs" style={{ background: proj.accent + '20', border: `1px solid ${proj.accent}40`, color: proj.accent }}>FEATURED</span>
        )}
        <span className="absolute top-4 right-4 px-2 py-1 rounded-md font-mono text-xs" style={{ background: 'rgba(0,0,0,.55)', color: 'rgba(255,255,255,.7)', border: '1px solid rgba(255,255,255,.1)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: hovered ? 1 : 0 }} className="absolute inset-0 flex items-center justify-center gap-3" style={{ background: 'rgba(0,0,0,.5)' }}>
          <a href={proj.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-lg font-mono text-xs font-medium" style={{ background: 'rgba(255,255,255,.1)', border: '1px solid rgba(255,255,255,.2)', color: '#fff' }}>
      ...
      <div className="p-6">
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <ArrowUpRight size={16} style={{ color: proj.accent, opacity: hovered ? 1 : .45, flexShrink: 0 }} />
        </div>
        <p className="text-sm leading-relaxed mb-5" style={{ color: 'rgba(255,255,255,.5)' }}>{proj.description}</p>
        <div className="flex flex-wrap gap-2 mb-4">
          {proj.tech.map(t => <span key={t} className="tech-tag" style={{ borderColor: hovered ? proj.accent + '30' : 'rgba(0,245,212,.12)', color: hovered ? proj.accent : '#00f5d4' }}>{t}</span>)}
        </div>
...
export default function Projects() {
  ...
  return (
    <div className="page-transition min-h-screen pt-24 pb-20 px-6 sm:px-10">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7 }} className="mb-12">
          ...
          <h1 className="font-display text-5xl sm:text-6xl font-bold mb-4 tracking-[-.04em]">Selected <span className="grad-text">Work</span></h1>
        ...
        <div className="flex items-center gap-3 mb-10 flex-wrap">
          {FILTERS.map(f => <motion.button key={f.id} onClick={() => setFilter(f.id)} whileHover={{ scale: 1.03 }} whileTap={{ scale: .97 }} className="px-5 py-2 rounded-xl font-mono text-sm" style={{ color: filter === f.id ? '#00f5d4' : 'rgba(255,255,255,.4)', background: filter === f.id ? 'rgba(0,245,212,.08)' : 'transparent', border: filter === f.id ? '1px solid rgba(0,245,212,.2)' : '1px solid rgba(255,255,255,.06)' }}>{f.label}</motion.button>)}
