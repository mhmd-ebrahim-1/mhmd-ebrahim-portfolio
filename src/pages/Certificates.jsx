...
const TIER_CONFIG = {
  ...
  learning: { label: 'Learning Activities', color: '#a78bfa', icon: BookOpen, bg: 'rgba(167,139,250,0.08)', border: 'rgba(167,139,250,0.2)' },
}

function CertCard({ cert, index, color }) {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h2 className="font-display font-semibold text-white text-xl">{config.label}</h2>
            <p className="font-mono text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
              {certs.length} certificate{certs.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        <div className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors group-hover:bg-white/5"
        ...
        {expanded && (
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              {certs.map((cert, i) => (
                <CertCard key={cert.id} cert={cert} index={i} color={config.color} />
              ))}
            </div>
          </motion.div>
        )}
...
export default function Certificates() {
  const topCerts = CERTIFICATES.filter(c => c.tier === 'top')
  const importantCerts = CERTIFICATES.filter(c => c.tier === 'important')
  const learningCerts = CERTIFICATES.filter(c => c.tier === 'learning')
    ...
    <div className="page-transition min-h-screen pt-24 pb-20 px-10">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <p className="font-mono text-xs mb-4" style={{ color: '#00f5d4', letterSpacing: '0.12em' }}>// 06 — ACHIEVEMENTS</p>
          ...
          <p className="text-base max-w-lg" style={{ color: 'rgba(255,255,255,0.4)', fontFamily: 'DM Sans' }}>
            {CERTIFICATES.length} certificates from NVIDIA, IBM, Huawei, Quantium, and more.
          </p>

          {/* Stats row */}
          <div className="flex gap-6 mt-8">
            {[
              { num: topCerts.length, label: 'Top Certs', color: '#00f5d4' },
              { num: importantCerts.length, label: 'Professional', color: '#0ea5e9' },
              { num: learningCerts.length, label: 'Learning', color: '#a78bfa' },
            ].map(s => (
              <div key={s.label} className="px-5 py-3 rounded-xl glass"
                style={{ border: `1px solid ${s.color}20` }}>
                <p className="font-display font-bold text-2xl" style={{ color: s.color }}>{s.num}</p>
                <p className="font-mono text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>{s.label}</p>
              </div>
            ))}
          </div>
        </motion.div>

        <CertSection tier="top" certs={topCerts} />
        <CertSection tier="important" certs={importantCerts} />
        <CertSection tier="learning" certs={learningCerts} />
