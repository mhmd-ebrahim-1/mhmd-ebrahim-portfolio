...
function CertCard({ cert, index }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 30 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ delay: index * 0.06, duration: 0.6, ease: [0.16, 1, 0.3, 1] }} whileHover={{ y: -4, scale: 1.01 }} className="glass rounded-2xl p-5 flex gap-4 items-start" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 font-display font-bold text-sm" style={{ background: cert.color + '18', border: `1px solid ${cert.color}30`, color: cert.color }}>{cert.issuer.slice(0, 2).toUpperCase()}</div>
...
function CertSection({ tier, certs }) {
  ...
  return (
    <motion.section ref={ref} initial={{ opacity: 0, y: 20 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12">
      <button onClick={() => setExpanded(e => !e)} className="w-full flex items-center justify-between mb-6 group text-left" aria-expanded={expanded}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: config.bg, border: `1px solid ${config.border}` }}><Icon size={16} style={{ color: config.color }} /></div>
          <div><h2 className="font-display font-semibold text-white text-lg sm:text-xl">{config.label}</h2><p className="font-mono text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>{certs.length} certificate{certs.length !== 1 ? 's' : ''}</p></div>
        </div>
        <div className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors group-hover:bg-white/5" style={{ border: '1px solid rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)' }}>{expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</div>
      </button>
      <AnimatePresence>
        {expanded && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }} className="overflow-hidden"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{certs.map((cert, i) => <CertCard key={cert.id} cert={cert} index={i} />)}</div></motion.div>}
      </AnimatePresence>
    </motion.section>
  )
}
...
export default function Certificates() {
  const groups = ['top', 'important', 'standard', 'learning'].map(tier => ({ tier, certs: CERTIFICATES.filter(c => c.tier === tier) }))
    ...
    <div className="page-transition min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-10">
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mb-14">
          ...
          <h1 className="font-display text-4xl sm:text-6xl font-bold mb-4 tracking-[-0.04em]">Certificates &amp;<br /><span className="grad-text">Achievements</span></h1>
